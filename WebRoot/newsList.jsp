...
<div align="center">
   <s:if test="#session.pageNum!=1">
		<a href="newsList.action?pageNum=1">首页</a>
  	</s:if>
  	<s:if test="#session.pageNum!=1">
  	<a href="newsList.action?pageNum=${session.pageNum-1}">上一页</a>
  	</s:if>
  	<c:forEach var="i" begin="1" end="${session.pageTotal}">
  		<a href="newsList.action?pageNum=<c:out value="${i}"/>">${i }</a>
  	</c:forEach>
  	<s:if test="#session.pageNum!=#session.pageTotal">
  	<a href="newsList.action?pageNum=${session.pageNum+1}">下一页</a>
  	</s:if>
  	<s:if test="#session.pageNum!=#session.pageTotal">
		<a href="newsList.action?pageNum=<s:property value="#session.pageTotal"/>">尾页</a>
  	</s:if>
